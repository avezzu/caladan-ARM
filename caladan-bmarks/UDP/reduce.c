...
static void *read_from(void *connection)
{
    ...
    while (atomic64_read(&counter) < N * amount_clients)
    {
        ...
        {
            atomic64_add_and_fetch(&table[k], buf[k]);
        }
    }
...
static void do_server(void *arg)
{
    ...
    table = calloc(buf_size, sizeof(atomic64_t));
    setup_connection(1, "129.132.85.196", &res, sizeof(atomic64_t) * buf_size);
...
int main(int argc, char *argv[])
{
    ...
    table = calloc(buf_size, sizeof(int));
