...
static void *read_from(void *connection)
{
    ...
    while (atomic64_read(&counter) < N * amount_clients)
    {
        ...
        {
            atomic64_add_and_fetch(&table[buf[k]], 1);
        }
    }
...
static void do_server(void *arg)
{
    ...
    table = calloc(amount_nummbers, sizeof(atomic64_t));
    setup_connection(1, "129.132.85.196", &res, sizeof(uint64_t) * amount_nummbers);
    ...
    uint64_t result[amount_nummbers];
    for (int i = 0; i < amount_nummbers; ++i)
    {
        result[i] = atomic64_read(&table[i]);
    }

    memcpy((&res)->buf, result, buf_size * sizeof(uint64_t));
